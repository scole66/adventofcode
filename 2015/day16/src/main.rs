...
impl Family {
    fn search(&self, needle: &MatchInput) -> Vec<i32> {
        self.aunts
            .iter()
            .filter(|&sue| {
                sue.children.is_none_or(|amt| amt == needle.children)
                    && sue.cats.is_none_or(|amt| amt == needle.cats)
                    && sue.samoyeds.is_none_or(|amt| amt == needle.samoyeds)
                    && sue.pomeranians.is_none_or(|amt| amt == needle.pomeranians)
                    && sue.akitas.is_none_or(|amt| amt == needle.akitas)
                    && sue.vizslas.is_none_or(|amt| amt == needle.vizslas)
                    && sue.goldfish.is_none_or(|amt| amt == needle.goldfish)
                    && sue.trees.is_none_or(|amt| amt == needle.trees)
                    && sue.cars.is_none_or(|amt| amt == needle.cars)
                    && sue.perfumes.is_none_or(|amt| amt == needle.perfumes)
            })
            .map(|sue| sue.id)
            .collect::<Vec<_>>()
    }
